...
# Script to create S3 loader file
# Run this from INSIDE your week3_capstone folder

echo "🔧 Creating S3 loader in src/loaders/..."
echo "Current directory: $(pwd)"

# Create s3_loader.py
echo "📝 Creating s3_loader.py..."
cat > src/loaders/s3_loader.py << 'EOF'
"""
S3 Data Loader
...
class S3Loader:
    ...
    def __init__(self):
        self.s3 = S3Helper(Config.S3_BUCKET_NAME, Config.AWS_REGION)
    ...
    def load_to_bronze(self, data: Dict[str, List[Dict]], source: str) -> Dict[str, str]:
        ...
        for data_type, records in data.items():
            ...
            if self.s3.upload_json(records, s3_key, metadata):
    ...
    def save_pipeline_metadata(self, metadata: Dict) -> str:
        ...
        if self.s3.upload_json(metadata, s3_key):
...
if [ $? -eq 0 ]; then
    echo "   ✅ s3_loader.py created successfully"
else
    echo "   ❌ Failed to create s3_loader.py"
fi

# Show results
echo ""
echo "📂 Files in src/loaders/:"
ls -la src/loaders/

echo ""
echo "📍 Location: $(pwd)/src/loaders/s3_loader.py"
