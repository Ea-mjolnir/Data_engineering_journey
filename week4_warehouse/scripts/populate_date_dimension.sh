...
# Generates and loads dates from 2020-01-01 to 2030-12-31
################################################################################
...
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}📅 Populating Date Dimension${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
...
# Create the SQL file
echo -e "\n${YELLOW}📝 Creating date population SQL file...${NC}"

mkdir -p ../sql/ddl

cat > ../sql/ddl/03_populate_date_dimension.sql << 'SQL_EOF'
-- ============================================================================
...
echo -e "${GREEN}✅ SQL file created at ../sql/ddl/03_populate_date_dimension.sql${NC}"

# Run the SQL file
echo -e "\n${YELLOW}⚙️  Populating date dimension in database...${NC}"
echo -e "${BLUE}--------------------------------------------------${NC}"

export PGPASSWORD=$DB_PASSWORD
psql -U $DB_USER -d $DB_NAME -h $DB_HOST -f ../sql/ddl/03_populate_date_dimension.sql
unset PGPASSWORD

# Check if successful
if [ $? -eq 0 ]; then
    echo -e "${BLUE}--------------------------------------------------${NC}"
    echo -e "${GREEN}✅ Date dimension populated successfully!${NC}"
    
    # Show summary
    echo -e "\n${YELLOW}📊 Date Dimension Summary:${NC}"
    export PGPASSWORD=$DB_PASSWORD
    ...
    SELECT 
        ...
        MAX(date) as latest,
        COUNT(*) FILTER (WHERE year = 2024) as dates_in_2024,
        COUNT(*) FILTER (WHERE is_holiday) as holiday_count
    FROM warehouse.dim_date;"
    unset PGPASSWORD
else
    echo -e "${RED}❌ Failed to populate date dimension${NC}"
    exit 1
fi

echo -e "\n${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}✅ Date dimension ready for time-based analysis!${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
